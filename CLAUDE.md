...
```json
{
  ...
  "properties": {
    ...
    "parentKey": { "type": "string", "description": "Parent issue key for creating a subtask (e.g., TEST-10)" }
  },
  "required": ["summary"]
}
```
...
3. If `parentKey` is provided and does not belong to `JIRA_PROJECT`, returns `isError: true`.
4. `issueType` defaults to `"Subtask"` when `parentKey` is set, otherwise `"Task"`. An explicit `issueType` always wins.
5. `storyPoints` maps to `fields.story_points`. Classic projects may store this as `customfield_10016` — see note below.
6. `parentKey` maps to `fields.parent = { key: parentKey }`.
7. Uses `jiraClient.sdk.issues.createIssue({ fields: { project, summary, description, issuetype, labels?, story_points?, parent? } })`.
8. Returns the created issue object (includes `key`, `id`, `self`).
...
```json
{
  ...
  "properties": {
    ...
    "storyPoints": { "type": "number", "description": "Story point estimate (maps to story_points field)" },
    "parentKey": { "type": "string", "description": "New parent issue key to re-parent a subtask (e.g., TEST-10)" }
  },
  "required": ["issueKey"]
}
```
...
2. If `parentKey` is provided and does not belong to `JIRA_PROJECT`, returns `isError: true`.
3. If `summary`, `description`, `assigneeAccountId`, `labels`, `storyPoints`, or `parentKey` provided → calls `sdk.issues.editIssue({ issueIdOrKey, fields })`. `assigneeAccountId` maps to `fields.assignee = { accountId: assigneeAccountId }`, `storyPoints` maps to `fields.story_points`, `parentKey` maps to `fields.parent = { key: parentKey }`.
